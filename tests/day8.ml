...
let test_parse_instruction () =
    let instruction = Instruction.parse "b inc -5 if a > -1" in
    match instruction with
    | None -> Alcotest.fail "could not parse"
    | Some instruction -> 
        Alcotest.(check string) "" instruction.register "b";
        match instruction.action with
        | Dec _-> Alcotest.fail "should be dec"
        | Inc i-> Alcotest.(check int) "" i (-5);
        match instruction.condition with
        | (Gt, "a", -1) -> ()
        | _ -> Alcotest.fail "should be Lr"


let test_largest_value_in_register input output () =
